...
private let kNormalColor :(CGFloat,CGFloat,CGFloat) = (138,138,138)
private let kSelectColor :(CGFloat,CGFloat,CGFloat) = (237, 76, 87)
...
extension PageTitleView {
    ...
    func reloadTitles(titleArray:[String]) -> Void {
        if titleArray.count == titles.count {
            for i in 0..<titles.count {
                titleLabels[i].text = titleArray[i]
            }
        }
    }
    
    func reloadTitlesColor(current: Int) {
        _ = titleLabels.map {
            $0.textColor = UIColor(r: kNormalColor.0, g: kNormalColor.1, b: kNormalColor.2)
        }
        
        let label = titleLabels[current]
        label.textColor = UIColor(r: kSelectColor.0, g: kSelectColor.1, b: kSelectColor.2)
        scrollLine.frame.origin.x = label.frame.origin.x + label.frame.width*0.1
        
        currentLabelIndex = current
    }
