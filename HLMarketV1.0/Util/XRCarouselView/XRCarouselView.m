...
+ (instancetype)carouselViewWithImageArray:(NSArray *)imageArray describeArray:(NSArray *)describeArray andScaleSize:(CGSize)scaleSize{
    XRCarouselView *carouselView = [[self alloc] init];
    carouselView.scaleSize = scaleSize;
...
#pragma mark -- 重新刷新数据
- (void)reloadWithImageArray:(NSArray *)imageArray describeArray:(NSArray *)describeArray{
    self.imageArray = imageArray;
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    if (!imageArray.count) {
        self.currImageView.image = [UIImage imageNamed:@"hlm_placeholder_rectangle.png"];
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            [_images addObject:[UIImage imageNamed:@"hlm_placeholder_rectangle.png"]];
...
- (void)downloadImages:(int)index {
    ...
    if (data) {
        ...
        _images[index] = [UIImage scaleToSize:image size:self.scaleSize];
        return;
    }
    //下载图片
    NSBlockOperation *download = [NSBlockOperation blockOperationWithBlock:^{
        ...
        //取到的data有可能不是图片
        if (image) {
            self.images[index] = [UIImage scaleToSize:image size:self.scaleSize];
            //如果下载的图片为当前要显示的图片，直接到主线程给imageView赋值，否则要等到下一轮才会显示
            if (_currIndex == index) [_currImageView performSelectorOnMainThread:@selector(setImage:) withObject:[UIImage scaleToSize:image size:self.scaleSize] waitUntilDone:NO];
