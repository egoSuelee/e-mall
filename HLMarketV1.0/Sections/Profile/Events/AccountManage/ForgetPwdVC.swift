...
class ForgetPwdVC: BaseViewController,UITextFieldDelegate {
    ...
    func getCodeAction() {
        ...
        do {
            ...
            if matches.count > 0 {
                //进行网络请求, 发送验证码
                AlamofireNetWork.required(urlString: "/Simple_online/Get_TelPhone_Code", method: .post, parameters: ["Tel": validateString, "reg_or_update":"1","cStoreNo":UserStoreManager.sharedManager.getStoreNo()], success: { [weak self] (results) in
                    ...
                    if results["resultStatus"] as! String == "000000" {
                        ...
                        self?.showHint(in: (self?.view)!, hint: "此用户还未注册")
                    }
    ...
    func nextRegisterAction() {
        ...
        if text! == self.verCode {
            let nextForgetVC = NextForgetPwdVC()
            nextForgetVC.tel = phoneTF.text!
            
            self.navigationController?.pushViewController(nextForgetVC, animated: true)
        } else {
